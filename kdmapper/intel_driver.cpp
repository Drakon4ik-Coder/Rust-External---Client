...
HANDLE intel_driver::Load()
{
	...
	return CreateFileW(xorstr_(L"\\\\.\\Nal"), GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
}
...
bool intel_driver::WriteToReadOnlyMemory(HANDLE device_handle, uint64_t address, void* buffer, uint32_t size)
{
	...
	{
		std::cout << xorstr_("[-] Failed to translate virtual address 0x") << reinterpret_cast<void*>(address) << std::endl;
		return false;
	}
	...
	{
		std::cout << xorstr_("[-] Failed to map IO space of 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
		return false;
	}
	...
	if (!UnmapIoSpace(device_handle, mapped_physical_memory, size))
		std::cout << xorstr_("[!] Failed to unmap IO space of physical address 0x") << reinterpret_cast<void*>(physical_address) << std::endl;
...
uint64_t intel_driver::AllocatePool(HANDLE device_handle, nt::POOL_TYPE pool_type, uint64_t size)
{
	...
	if (!kernel_ExAllocatePool)
		kernel_ExAllocatePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")), xorstr_("ExAllocatePool"));
...
bool intel_driver::FreePool(HANDLE device_handle, uint64_t address)
{
	...
	if (!kernel_ExFreePool)
		kernel_ExFreePool = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("ntoskrnl.exe")), xorstr_("ExFreePool"));
...
bool intel_driver::GetNtGdiDdDDIReclaimAllocations2KernelInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr, uint64_t* out_kernel_original_function_address)
{
	...
	{
		const uint64_t kernel_NtGdiDdDDIReclaimAllocations2 = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("win32kbase.sys")), xorstr_("NtGdiDdDDIReclaimAllocations2"));
...
bool intel_driver::GetNtGdiGetCOPPCompatibleOPMInformationInfo(HANDLE device_handle, uint64_t* out_kernel_function_ptr, uint8_t* out_kernel_original_bytes)
{
	...
	{
		const uint64_t kernel_NtGdiGetCOPPCompatibleOPMInformation = GetKernelModuleExport(device_handle, utils::GetKernelModuleAddress(xorstr_("win32kfull.sys")), xorstr_("NtGdiGetCOPPCompatibleOPMInformation"));
